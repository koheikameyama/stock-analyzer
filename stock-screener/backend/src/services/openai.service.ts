...
export class OpenAIService {
  ...
  private static generateAnalysisPrompt(input: StockAnalysisInput): string {
    ...
    return `あなたは経験豊富な株式アナリストです。以下のデータを基に、この銘柄に対する投資推奨（Buy/Sell/Hold）と信頼度スコア（0-100%）、理由を提供してください。
          ...
          messages: [
            {
              role: 'system',
              content:
                'あなたは株式アナリストです。財務データと株価データを分析し、投資推奨を提供します。常にJSON形式で回答してください。',
            },
            {
              role: 'user',
              content: prompt,
