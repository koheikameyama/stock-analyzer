...
バージョンラベル（major/minor/patch）を付ける場合は、PR本文に必ず「## ユーザー向け説明」セクションを追加してください。
...
🔔 日次更新完了時にプッシュ通知
📱 トップページから通知の有効化/無効化を簡単に切り替えられます
⏰ 毎日18:00の分析完了を見逃さずに確認できます
```

- **絵文字を活用**: 各行の先頭に関連する絵文字を付けると、リリースノートが見やすくなります
- **改行で複数の説明**: 複数の機能がある場合は改行で区切ってください（すべて表示されます）
- **ユーザー目線**: ユーザーが理解できる言葉で、何が変わったかを説明
- **リリースノートに表示**: この説明がGitHub Releaseとリリースモーダルに表示されます
- **ラベルなしの場合は不要**: 内部改善の場合は「ユーザー向け説明」は不要
