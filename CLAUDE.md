...
**🚨 重要: バージョンラベルを付ける場合は「ユーザー向けの変更」セクションが必須です**

`version:major` または `version:minor` ラベルを付ける場合、**必ずPR本文に以下のセクションを追加してください：**

```markdown
### ユーザー向けの変更
- **機能名**: ユーザーにとっての価値を説明（技術的な詳細ではなく、ユーザーメリットを記載）
```

**例:**
```markdown
### ユーザー向けの変更
- **プッシュ通知設定の改善**: ON/OFF状態が一目でわかるトグルスイッチに変更しました。現在の設定状態が視覚的に明確になり、誤操作を防げます
```

**なぜ必要？**
- このセクションがないと、デプロイPRに転記されず、リリースノートが生成されません
- ユーザーに新機能が通知されず、せっかくの改善が気づかれません

**チェックリスト:**
- [ ] `version:minor` または `version:major` ラベルを付けた？
- [ ] PR本文に「### ユーザー向けの変更」セクションを追加した？
- [ ] ユーザーメリットを明確に記載した？（技術用語ではなく、価値を説明）
...
batch/のみの変更でも、ユーザーに価値が伝わる変更であれば`version:patch`ラベルを付けてください。

- ✅ **付けるべき例**:
  - AI分析精度の向上（株価トレンド分析機能追加など）
  - データ取得の信頼性向上
  - 分析速度の大幅改善
- ❌ **付けないべき例**:
  - コードのリファクタリング
...
- 「バッチの変更ですが、ユーザーに価値がある改善なので `version:patch` ラベルを付けますか？」
- 「技術的なバグ修正なのでラベルなしで良いですか？（次のminorリリースに含まれます）」
...
**重要: batch/のみの変更の場合、create-release.ymlは自動実行されません。**

- web/の変更あり → Railwayデプロイ → `deployment_status`イベント → `create-release.yml`自動実行
...
これにより、batch/のみの変更でもversion:patchラベル付きのPRがあればリリースが作成されます。

### PR作成の運用

**PR作成は手動で行い、本文は空で作成してください。**
...
2. **PRを空の本文で作成**
   ...
   ```bash
   gh pr create
   # インタラクティブモードで、本文は空のまま作成
   ```

3. **AI分析が自動実行される**
   - `ai-pr-assistant.yml`ワークフローが自動的に実行されます
   - バージョンラベルが判定・付与されます
   - PR本文が自動生成・更新されます
   - 判定結果がコメントで通知されます
...
- コミット完了後、PR作成を提案してください
- 既存のPRがあれば更新、なければ新規作成を提案
- ユーザーの意思を確認してから実行

**提案例:**
```
コミットが完了しました！PRを作成しますか？

現在のブランチ: feature/example
コミット数: origin より1つ先行
...
- ❌ **PR本文を手動で書かない**: AIが自動生成します
- ✅ **空の本文で作成**: `--body ""`を指定
- ✅ **タイトルはわかりやすく**: コミットメッセージを元に設定
- ✅ **AI分析を待つ**: PR作成後、約20秒でAIが本文を生成します
